...
flights %>% 
  group_by(tailnum) %>%
  summarize(mean(arr_delay + dep_delay, na.rm = T))


flights %>%
  mutate(arrived = complete.cases(arr_delay), 
         on_time = arr_delay <=5, 
         is_it_good = arrived & on_time) %>% group_by(tailnum)

summarize(proportion_on_time = mean(is_it_good),number_of_flights = n())%>%
  arrange(desc(proportion_on_time)) %>% View
```
...
flights%>%
  group_by(dest)%>%
  summarize(longest_flight = max(air_time, na.rm = T),
            shortest_flight = min(air_time, na.rm =T),
            number_of_flights = n()) %>%
  mutate(difference = longest_flight- shortest_flight,
         ratio = shortest_flight/longest_flight)
